...
@section('css')
<link rel="stylesheet" type="text/css" href="{{ url('/admin_theme/app-assets/css/pages/users.min.css') }}">

  <link rel="stylesheet" type="text/css" href="{{ url('/admin_theme/assets/css/style.css') }}">
<!-- <style type="text/css">
    .tree, .tree ul {
    margin:0;
    padding:0;
...
@section('content')
        <div class="content-header-left col-md-6 col-12 mb-2 breadcrumb-new">
          <h3 class="content-header-title mb-0 d-inline-block">Role Update</h3>
                      ...
                      <div class="form-body">
                        ...
                        <div class="row">
                          ...
                          <div class="form-group col-md-6 mb-2">                           
                              ...
                              <input type="text" id="role" readonly name="role_name" class="form-control" value="{{$role->role_name}}" required autocomplete="off" />
                          </div>
                          @endif
                          @if($role->id != '1')
                          <div class="form-group col-md-6 mb-2">                           
                              <label for="projectinput3">Role</label>                         
                              <input type="text" id="role"  name="role_name" class="form-control" value="{{$role->role_name}}" required autocomplete="off" />
                          </div>
                          @endif
                          ...
                          <div class="form-group col-md-6 mb-2" id="list">
                            ...
                            @foreach($menus as $menu)
                               @if($menu->menu_parent == '0')
                                <li class="column"> <input  type="checkbox" id="{{$menu->id}}" name="checkboxlist[]" @if(in_array($menu->id, $existrole_menu)) checked @endif value="{{$menu->id}}"><a href="#">{{$menu->menu_name}}</a>
                                     @if(count($menu->sub_menus) != '0')
                                      <ul class="firstul">
                                       @foreach($menu->sub_menus as $submenu)
                               
                                        <li class="secondli"><span><input  type="checkbox" id="{{$submenu->id}}" @if(in_array($submenu->id, $existrole_menu)) checked @endif name="checkboxlist[]" value="{{$submenu->id}}">{{$submenu->menu_name}}</span>
                                          @if(count($submenu->sub_menus) != '0')
                                          <ul>                   
                                                  @foreach($submenu->sub_menus as $subsubmenu)
                                                  <li><span><input type="checkbox"  id="{{$subsubmenu->id}}" @if(in_array($subsubmenu->id, $existrole_menu)) checked @endif name="checkboxlist[]" value="{{$subsubmenu->id}}">{{$subsubmenu->menu_name}}</span>
                                                  </li>
                                                  @endforeach
                                            </ul>
                                            @endif
                                                                      ...
                                                                      @foreach($menu->sub_menus as $submenu)
                                                                        ...
                                                                        <li class="secondli">
                                                                               ...
                                                                               <ul class="thirdli">
                                                                                  ...
                                                                                  <li class="thirdlilist">
                                                                                     <label class="container-check">
                                                                                        ...
                                                                                        <input type="checkbox"  id="{{$subsubmenu->id}}" @if(in_array($subsubmenu->id, $existrole_menu)) checked @endif name="checkboxlist[]" value="{{$subsubmenu->id}}">
                                                                          ...
                                                                          @else
                                                                                     ...
                                                                                     <label class="container-check">
                                                                                        <h3>{{$submenu->menu_name}}</h3>
                                                                                        <input type="checkbox" id="{{$submenu->id}}" @if(in_array($submenu->id, $existrole_menu)) checked @endif name="checkboxlist[]" value="{{$submenu->id}}">
                                                                                        <span class="checkmark"></span>
                                                                                     </label>
